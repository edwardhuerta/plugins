...
public class CameraPlugin implements MethodCallHandler {
    ...
    public void onMethodCall(MethodCall call, final Result result) {
        switch (call.method) {
            ...
            case "zoomByFactor": {
                double zoomlvl = call.argument("zF");
                camera.setCurrentZoom(zoomlvl);
                break;
            }
            case "dispose": {
                if (camera != null) {
...
//                result.error("captureFailure", reason, null);
        ...
        private void setCurrentZoom(double zoomLevel) {
            float zoomLevelFl = (float) zoomLevel;
            zoomRect = getZoomRect(zoomLevelFl);
            if (zoomRect != null) {
                try {
        ...
        private float getMaxZoom() {
            try {
                return (cameraManager.getCameraCharacteristics(cameraName).get(CameraCharacteristics.SCALER_AVAILABLE_MAX_DIGITAL_ZOOM)) * 10;
            } catch (Exception e) {
                Log.e(TAG, "Error during camera init");
                return -1;
            }
